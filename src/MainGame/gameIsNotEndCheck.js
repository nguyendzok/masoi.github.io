...
exports = (gamef, bot, userRoom, callback) => {
  gamef.getRoom(userRoom).gameIsEnd(async (winner) => {
    if (winner === 0) {
      ...
      await roomChatAll(bot, gamef.getRoom(userRoom).players, 0, [`🏆Trò chơi đã kết thúc...\n${winner === -1 ? '🐺SÓI' : winner === 1 ? '💩DÂN': '👼CẶP ĐÔI'} thắng!`, `🎮Bạn có thể sẵn sàng để bắt đầu chơi lại, hoặc tiếp tục trò chuyện với các người chơi khác trong phòng!`]);
