...
module.exports = async (gamef, bot, userRoom) => {
    gamef.getRoom(userRoom).roleIsDone(async () => {
        ...
        if (gamef.getRoom(userRoom).saveOrKill < 0) {
            ...
            await roomChatAll(bot, gamef.getRoom(userRoom).players, 0, `👻Đã treo cổ ${deathTxt}! Mọi người đi ngủ`);
            gamef.getRoom(userRoom).newLog(`👻Mọi người đã treo cổ ${deathRoleTxt} *${deathTxt}* với ${(gamef.getRoom(userRoom).aliveCount() + 1 + gamef.getRoom(userRoom).saveOrKill) / 2} tha/${(gamef.getRoom(userRoom).aliveCount() + 1 - gamef.getRoom(userRoom).saveOrKill) / 2} treo`);
        } else {
